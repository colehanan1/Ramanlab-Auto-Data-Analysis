...
Usage examples:
    python scripts/analysis/collect_eye_prob_coords.py
    python scripts/analysis/collect_eye_prob_coords.py --verbose --out /path/to/all_eye_prob_coords_wide.csv
    python scripts/analysis/collect_eye_prob_coords.py --sources /path/a /path/b --dry-run
    python scripts/analysis/collect_eye_prob_coords.py --out results.csv --npy-out results.npy --json-out results.json
"""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    tool_cfg = config_data.get("tools", {}).get("collect_eye_prob_coords", {})
    if not isinstance(tool_cfg, dict):
        tool_cfg = {}

    sources = args.sources or tool_cfg.get("sources") or DEFAULT_SOURCES
    output_csv = args.out or tool_cfg.get("output_csv") or DEFAULT_OUTPUT
    if not sources:
        raise SystemExit("No sources configured. Pass --sources or set tools.collect_eye_prob_coords.sources.")
    if not output_csv:
        raise SystemExit("No output path configured. Pass --out or set tools.collect_eye_prob_coords.output_csv.")

    args.sources = [str(Path(src).expanduser()) for src in sources]
    args.out = str(Path(output_csv).expanduser())

    global FPS_VALUE, MAX_FRAMES
    FPS_VALUE = int(tool_cfg.get("fps", FPS_VALUE))
    MAX_FRAMES = int(tool_cfg.get("max_frames", MAX_FRAMES))
