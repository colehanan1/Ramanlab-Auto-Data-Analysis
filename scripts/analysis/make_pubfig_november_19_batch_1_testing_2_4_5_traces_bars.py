#!/usr/bin/env python3
"""Standalone figure: testing 2/4/5 traces + PER% bars for november_19_batch_1 fly 1 (opto_3-oct).

Traces are labeled as 3-OCTANOL (blue + bold).
Bars show PER% (manual values) across testing odors; 3-Octanol bar is blue + bold.
"""
...
OUT_PATH = ROOT / "Data/imagesForFigures/imagesForOptoHEXTesting/november_19_batch_1_testing_2_4_5_traces_bars.png"

FLY_NAME = "november_19_batch_1"
FLY_NUMBER = "1"

TRACE_TRIALS = [2, 4, 5]
TRACE_COLORS = ["#1f77b4", "#ff7f0e", "#2ca02c"]
ODOR_LABEL = "3-Octanol"
...
def _display_odor(dataset_canon: str, trial_label: str) -> str:
    """Mapping for opto_3-oct testing trials."""
    number = _trial_num(trial_label)
    dataset = str(dataset_canon)

    if dataset == "opto_3-oct":
        if number in (1, 3):
            return "Hexanol"
        if number in (2, 4, 5):
            return "3-Octonol"
...
def main() -> None:
    ...
    if df.empty:
        raise RuntimeError("No testing trials found for november_19_batch_1 fly 1.")
    ...
    if trace_rows.empty:
        raise RuntimeError("No testing_2/4/5 rows found for november_19_batch_1 fly 1.")
    ...
    bar_labels = [label for _, label in BAR_ORDER]
    ...
    fig.suptitle(
        "Conditioned Fruit Fly — Testing 2, 4, 5 Traces + PER% Bars (3-Octanol Trained)",
        fontsize=14,
        weight="bold",
        y=0.96,
    )
