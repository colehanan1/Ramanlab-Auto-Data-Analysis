#!/usr/bin/env python3
"""Standalone figure: testing 2/4/5 traces + PER% bars for october_10_batch_1 fly 2.
...
OUT_PATH = ROOT / "Data/imagesForFigures/imagesForOptoHEXTesting/october_10_batch_1_testing_2_4_5_traces_bars.png"

FLY_NAME = "october_10_batch_1"
FLY_NUMBER = "2"
...
def _display_odor(dataset_canon: str, trial_label: str) -> str:
    """Minimal mapping for opto_hex / hex_control testing trials."""
    number = _trial_num(trial_label)
    dataset = str(dataset_canon)
    if dataset in ("opto_hex", "hex_control"):
        if number in (1, 3):
...
def main() -> None:
    ...
    if df.empty:
        raise RuntimeError("No testing trials found for october_10_batch_1 fly 2.")
    ...
    if trace_rows.empty:
        raise RuntimeError("No testing_2/4/5 rows found for october_10_batch_1 fly 2.")
    ...
    fig.suptitle(
        "Conditioned Fruit Fly — Testing 2, 4, 5 Traces + PER% Bars (Hexanol Trained)",
        fontsize=14,
        weight="bold",
        y=0.96,
    )
