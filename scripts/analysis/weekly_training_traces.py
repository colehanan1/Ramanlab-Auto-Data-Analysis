...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "results" / "weekly-training-envelopes"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(str(LOG_DIR / 'weekly_training_traces.log')),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Weekly training trace analysis")
    parser.add_argument(
        "--config",
        default=str(Path("config") / "config.yaml"),
        help="Path to pipeline configuration YAML.",
    )
    parser.add_argument("--csv-path", default=None, help="Path to the training-wide CSV.")
    parser.add_argument("--output-dir", default=None, help="Directory for weekly outputs.")
    args = parser.parse_args()

    config_data = load_raw_config(args.config)
    tools_cfg = config_data.get("tools", {}).get("weekly_training_traces", {})
    if not isinstance(tools_cfg, dict):
        tools_cfg = {}

    training_csv = ""
    wide_cfg = config_data.get("analysis", {}).get("combined", {}).get("wide", {})
    if isinstance(wide_cfg, dict):
        for entry in wide_cfg.get("trial_type_exports", []) or []:
            if isinstance(entry, dict) and entry.get("trial_type") == "training":
                training_csv = entry.get("output_csv", "")
                break

    csv_path_value = args.csv_path or tools_cfg.get("csv_path") or training_csv
    output_dir_value = args.output_dir or tools_cfg.get("output_dir") or str(DEFAULT_OUTPUT_DIR)

    if not csv_path_value:
        raise SystemExit(
            "CSV path not configured. Provide --csv-path or set tools.weekly_training_traces.csv_path."
        )
