...
Usage:
    python scripts/convert/compress_videos.py [--delete-raw] [--dry-run]
"""
import argparse
import logging
import subprocess
...
def resolve_directories(config_path: str | Path, override: Sequence[str] | None) -> List[str]:
    if override:
        return list(override)

    settings = load_settings(config_path)
    if isinstance(settings.main_directory, list):
        return [str(path) for path in settings.main_directory]
    if settings.main_directory:
        return [str(settings.main_directory)]
    return []
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Override the directories to scan (defaults to main_directory in the config).",
    )
    ...
    directories = resolve_directories(args.config, args.dirs)
    if not directories:
        log.error("No directories configured. Provide --dirs or set main_directory in the config.")
        return
    videos = find_videos(directories)
