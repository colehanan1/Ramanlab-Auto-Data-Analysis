...
def _run_pipeline(config_path: Path, allow_cpu: bool) -> None:
    ...
    try:
        ...
        if exc.returncode != -11 or allow_cpu:
            raise

        print("[analysis] pipeline failed with SIGSEGV; retrying with ALLOW_CPU=true")
        env = os.environ.copy()
        env["ALLOW_CPU"] = "true"
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    allow_cpu = (
        os.getenv("ALLOW_CPU", str(data.get("allow_cpu", False))).lower() == "true"
    )

    _run_pipeline(config_path, allow_cpu)
