...
def _process_fly_dir(fly_dir: Path, cfg: Settings) -> None:
    """Normalize all distance CSVs under a single fly directory.

    This is a module-level (picklable) worker function used by parallel_map.
    All values derived from cfg that are needed per fly_dir are read from cfg
    directly inside this function so workers are self-contained.
    """
